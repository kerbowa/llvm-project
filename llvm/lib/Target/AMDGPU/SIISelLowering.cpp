...
void SITargetLowering::allocatePreloadKernArgSGPRs(
  ...
  for (auto &N : MD->operands()) {
    ...
    for (; InIdx < Ins.size() && Ins[InIdx].isOrigArg() &&
      ...
      // TODO: Will previous arg always be a single SGPR in this case?
      ...
      // TODO: Switch to utility.
      const TargetRegisterClass *RC = nullptr;
      switch (NumAllocSGPRs) {
      case 1:
...
void SITargetLowering::allocateLDSKernelId(CCState &CCInfo, MachineFunction &MF,
                                           const SIRegisterInfo &TRI,
                                           SIMachineFunctionInfo &Info) const {
  // Allways allocate this last since it is not part of the ABI so will not be
  // manged by firmware.
  if (Info.hasLDSKernelId()) {
      ...
      if (Arg.isOrigArg() &&
        ...
        if (MemVT.getStoreSize() < 4 && Alignment < 4) {
          ...
          if (PreloadRegs.size() == 1) {
            ...
            for (auto Reg : PreloadRegs) {
              Register VReg = MRI.getLiveInVirtReg(Reg);
              SDValue Copy = DAG.getCopyFromReg(C, DL, VReg, MVT::i32);
              C = Copy.getValue(1);
              SDValue Cast = DAG.getBitcast(MVT::i32, Copy);
              Elts.push_back(Cast);
            }
            NewArg =
